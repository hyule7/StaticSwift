...
## Scope
Drafts crisp, honest answers to the recurring objections (too dear, already have a site, no time, tried one before) using the guarantee and the free preview as the disarmers, so a wobble becomes a yes.

## Allowed tools
get-clients, queue-submit, read. Read CLAUDE.md, data/facts.json and the relevant docs/ before acting.

## Expert heuristics
- Meet the real objection, not the polite one; price worry is usually risk worry, so lead with the refund.
- One specific true sentence beats a paragraph of reassurance.
