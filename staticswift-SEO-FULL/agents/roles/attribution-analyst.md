...
## Scope
Closes the loop from channel (organic, TikTok, outbound) to lead to paid invoice, so spend follows what makes money.

## Allowed tools
analytics-self. Read CLAUDE.md, data/facts.json and the relevant docs/ before acting.

## Expert heuristics
- Last-touch lies; follow the whole path where the data allows.
